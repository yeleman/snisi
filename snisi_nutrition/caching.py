...
def update_cluster_caches_for(cluster):
    ...
    return {
        'nutrition_routine': update_nutrition_monthly_routine_cache,
        'nutrition_routine_sms': update_nutrition_monthly_routine_sms_cache
    }.get(cluster.slug, lambda c: False)(cluster)


def update_nutrition_monthly_routine_cache(cluster):
    # update member's list JSON cache.
    return generate_json_cache_from_cluster_members(
        cluster, skip_slugs=['health_area', 'vfq'])


def update_nutrition_monthly_routine_sms_cache(cluster):
