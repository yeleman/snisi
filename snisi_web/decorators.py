...
def user_role_within(role_list):
    ...
    def decorator(target):
        def wrapper(request, *args, **kwargs):
            roles = role_list if isinstance(role_list, collections.Iterable) else [role_list]
...
def user_location_type_within(type_slug_list):
    """ decorator asserting user's location__type__slug is with the provided list """
    def decorator(target):
        def wrapper(request, *args, **kwargs):
            type_slugs = type_slug_list if isinstance(type_slug_list, collections.Iterable) else [type_slug_list]
